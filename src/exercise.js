...
(() => {
  ...
  function answerFromScript(left, right, options) {
    const lines = state.situationScript.map((entry) => entry.line).filter(Boolean);
    if (!lines.length) return '';
  ...
  function mountSituationAutocomplete() {
    ...
    state.situationGapItems = controls.map((control) => {
      ...
      const answer = answerFromScript(left, right, options);
